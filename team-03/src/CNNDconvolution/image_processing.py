...
def reduce_image_shape(image : np.ndarray, new_shape : tuple) -> np.ndarray:
    (pad_layers, pad_rows, pad_cols) = (
        np.array(image.shape) - np.array(new_shape)
    ) // 2
    
    pad_layers = pad_layers if pad_layers >= 0 else 0
    pad_rows = pad_rows if pad_rows >= 0 else 0
    pad_cols = pad_cols if pad_cols >= 0 else 0

    new_img = image[pad_layers:new_shape[0]+pad_layers, pad_rows : new_shape[1] + pad_rows, pad_cols:new_shape[2]+pad_cols]
