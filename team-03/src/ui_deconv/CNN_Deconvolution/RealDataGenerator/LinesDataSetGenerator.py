...
class LinesDataSetGenerator:
    ...
    def DrawLines(self, series, blured_mask, lines_per_image, rad_x, rad_y, rad_z, intensity_interval = (0.65, 1.), LOWER_COEF = 1.):
        # 1 - генерируем холсты
        ...
        for i in range(lines_per_image):
            ...
            omega = uniform(0, np.pi / 2.) if blured_mask.shape[0] != 1 else 0
            ...
            new_blur_mask = blur_mask / np.sum(blur_mask)
            new_clear_mask = clear_mask / np.sum(clear_mask)
    ...
    def GenerateLinesModel(self, blured, series = 1, lines_per_image = 1, 
        ...
        for blur_mask in blured:
            ...
            micro_data = self.DrawLines(series, blur_mask, lines_per_image, rad_x, rad_y, rad_z, LOWER_COEF=LOWER_COEF)
